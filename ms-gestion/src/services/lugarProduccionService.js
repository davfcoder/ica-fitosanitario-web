...
const notificar = require('../utils/notificarInspeccion');
...
class LugarProduccionService {
    ...
    async crearSolicitud(datos, usuarioSolicitante) {
        ...
        await notificar({
            ...
            tipo: 'lugar_nuevo',
            titulo: 'Nueva solicitud de lugar de producción',
            mensaje: `${usuarioSolicitante.nombres || 'Un productor'} registró un nuevo lugar de producción pendiente de revisión.`,
            metadata: { id_lugar_produccion: nuevoLugar.id_lugar_produccion },
            ruta_destino: '/admin/lugares'
        });
    ...
    async corregirSolicitud(id, datos, usuarioSolicitante) {
        ...
        // Avisar a admins de la solicitud reenviada
        await notificar({
            para_admins: true,
            id_usuario_remitente: usuarioSolicitante.id_usuario,
            tipo: 'lugar_nuevo',
            titulo: 'Lugar de producción reenviado tras corrección',
            mensaje: `${usuarioSolicitante.nombres || 'Un productor'} corrigió y reenvió un lugar de producción.`,
            metadata: { id_lugar_produccion: id },
            ruta_destino: '/admin/lugares'
        });
    ...
    async solicitarEdicion(id, datos, usuario) {
        ...
        await notificar({
            ...
            tipo: 'lugar_cambio_solicitado',
            titulo: 'Solicitud de edición de lugar de producción',
            mensaje: `Un productor solicitó editar el lugar "${lugar.nom_lugar_produccion}".`,
            metadata: { id_lugar_produccion: id },
            ruta_destino: '/admin/lugares'
        });
    ...
    // === PRODUCTOR: Solicitar cancelación ===
    async solicitarCancelacion(id, datos, usuario) {
        ...
        await notificar({
            ...
            id_usuario_remitente: usuario.id_usuario,
            tipo: 'lugar_cambio_solicitado',
            titulo: 'Solicitud de cancelación de lugar de producción',
            mensaje: `Un productor solicitó cancelar el lugar "${lugar.nom_lugar_produccion}".`,
            metadata: { id_lugar_produccion: id },
            ruta_destino: '/admin/lugares'
        });
    ...
    // === ADMIN: Aprobar cambio solicitado ===
    async aprobarCambio(id, datos, usuario) {
        const lugar = await lugarProduccionRepository.findById(id);
        if (!lugar) throw { status: 404, message: 'Lugar de producción no encontrado' };
        ...
        if (lugar.estado === 'en_edicion') {
            ...
            await notificar({
                id_usuario_destinatario: lugar.id_usuario_productor,
                id_usuario_remitente: usuario.id_usuario,
                tipo: 'lugar_cambio_resuelto',
                titulo: 'Solicitud de edición aprobada',
                mensaje: `Su solicitud de edición sobre "${lugar.nom_lugar_produccion}" fue aprobada. Ya puede realizar los cambios.`,
                metadata: { id_lugar_produccion: id },
                ruta_destino: '/productor/lugares'
            });
            return await lugarProduccionRepository.findById(id);
        }
        ...
        if (lugar.estado === 'en_cancelacion') {
            ...
            await notificar({
                ...
                id_usuario_remitente: usuario.id_usuario,
                tipo: 'lugar_cambio_resuelto',
                titulo: 'Cancelación de lugar aprobada',
                mensaje: `Su solicitud de cancelación de "${lugar.nom_lugar_produccion}" fue aprobada.`,
                metadata: { id_lugar_produccion: id },
                ruta_destino: '/productor/lugares'
            });
            return await lugarProduccionRepository.findById(id);
    ...
    async rechazarCambio(id, datos, usuario) {
        const lugar = await lugarProduccionRepository.findById(id);
        if (!lugar) throw { status: 404, message: 'Lugar de producción no encontrado' };
        ...
        await notificar({
            ...
            mensaje: `Su solicitud de cambio sobre "${lugar.nom_lugar_produccion}" fue rechazada. Motivo: ${datos.observaciones}`,
            metadata: { id_lugar_produccion: id },
            ruta_destino: '/productor/lugares'
        });
    ...
    // === Consultas ===
    async listarLugares() { return await lugarProduccionRepository.findAll(); }
    async listarLugaresPorProductor(idProductor) { return await lugarProduccionRepository.findByProductor(idProductor); }
    async listarSolicitudesPendientes() { return await lugarProduccionRepository.findByEstado('pendiente'); }
    ...
    async aprobarSolicitud(id, datos, usuario) {
        const lugar = await lugarProduccionRepository.findById(id);
        if (!lugar) throw { status: 404, message: 'Lugar de producción no encontrado' };
        ...
        await notificar({
            ...
            id_usuario_remitente: usuario.id_usuario,
            tipo: 'lugar_aprobado',
            titulo: 'Lugar de producción aprobado',
            mensaje: `Su lugar "${lugar.nom_lugar_produccion}" fue aprobado. Registro ICA: ${datos.nro_registro_ica}`,
            metadata: { id_lugar_produccion: id },
            ruta_destino: '/productor/lugares'
        });
    ...
    async rechazarSolicitud(id, datos, usuario) {
        const lugar = await lugarProduccionRepository.findById(id);
        if (!lugar) throw { status: 404, message: 'Lugar de producción no encontrado' };
        if (lugar.estado !== 'pendiente') {
        ...
        await notificar({
            id_usuario_destinatario: lugar.id_usuario_productor,
            id_usuario_remitente: usuario.id_usuario,
            tipo: 'lugar_rechazado',
            titulo: 'Lugar de producción rechazado',
            mensaje: `Su lugar "${lugar.nom_lugar_produccion}" fue rechazado. Motivo: ${datos.observaciones}`,
            metadata: { id_lugar_produccion: id },
            ruta_destino: '/productor/lugares'
        });
    ...
    async devolverSolicitud(id, datos, usuario) {
        const lugar = await lugarProduccionRepository.findById(id);
        if (!lugar) throw { status: 404, message: 'Lugar de producción no encontrado' };
        if (lugar.estado !== 'pendiente') {
        ...
        await notificar({
            ...
            id_usuario_remitente: usuario.id_usuario,
            tipo: 'lugar_devuelto',
            titulo: 'Lugar de producción devuelto para corrección',
            mensaje: `Debe corregir su lugar "${lugar.nom_lugar_produccion}". Observaciones: ${datos.observaciones}`,
            metadata: { id_lugar_produccion: id },
            ruta_destino: '/productor/lugares'
        });
